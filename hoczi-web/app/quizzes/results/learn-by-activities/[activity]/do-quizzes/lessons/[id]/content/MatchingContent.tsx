...
function QuestionView({ q, index, isLast, onResult, onNext, onFinish }: {
    ...
    function getLeftClass(item: string) {
        if (checked) {
            ...
            return isCorrect ? 'border-green-400 bg-green-50 text-green-800'
                : matches[item] ? 'border-red-400 bg-red-50 text-red-800'
    ...
    function getRightClass(item: string) {
        if (checked) {
            ...
            return isCorrect ? 'border-green-400 bg-green-50 text-green-800'
                : left ? 'border-red-400 bg-red-50 text-red-800'
                : 'border-gray-200 bg-gray-50 text-gray-400';
        }
        if (matchedLeftByRight[item]) {
            const left = matchedLeftByRight[item];
            return PAIR_COLORS[pairIndex(left) % PAIR_COLORS.length] + ' border-2';
        }
            ...
            <div className="flex items-center gap-3">
                ...
                {checked && (
                    <button onClick={isLast ? onFinish : onNext}
                        className={`px-5 py-2.5 text-white text-sm font-semibold rounded-xl transition-colors ml-auto ${allCorrect ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-700 hover:bg-gray-800'}`}>
                        {isLast ? 'Finish ✓' : 'Next →'}
                    </button>
                )}
            </div>
...
export function MatchingContent({ activities }: ContentProps) {
    ...
    if (questions.length === 0) {
        return <div className="flex items-center justify-center min-h-[300px] text-gray-400 text-sm">No matching activities found.</div>;
    }
