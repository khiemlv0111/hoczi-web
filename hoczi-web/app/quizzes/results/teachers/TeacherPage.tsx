...
function CreateQuizModal({ onClose, onCreate }: {
    ...
    const submit = async () => {
        ...
        try {
            ...
            LessonService.createQuizAssignment(payload).then((res) => {
                onCreate(res?.data ?? res)
            })


            
            // const res = await postRequest('/api/quizzes/create-quiz', payload, true)
            // onCreate(res?.data ?? res)
...
function QuizDetailModal({ quiz, onClose, onUpdated }: {
    ...
    const save = async () => {
        ...
        try {
            ...
            let question_ids = Array.from(selectedIds)
            console.log('SELECTED', selectedIds);
            
            // await postRequest(`/api/quizzes/${quiz.id}/add-questions`, {
            //     question_ids: Array.from(selectedIds),
            // }, true)
            // onUpdated({ ...quiz, question_count: (quiz.question_count ?? 0) + selectedIds.size })
            // onClose()
...
function QuizzesTab({ classes }: { classes: ClassItem[] }) {
