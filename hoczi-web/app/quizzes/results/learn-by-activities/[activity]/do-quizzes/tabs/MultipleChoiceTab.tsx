...
export function MultipleChoiceTab({ lessons, activity, loading }: TabProps) {
    if (loading) {
        return (
    ...
    if (lessons.length === 0) {
        return (
            <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
                ...
                <p className="text-sm text-gray-500">No Multiple Choice lessons found.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-3">
            {lessons.map((lesson, index) => (
                <Link
                    key={lesson.id}
                    href={`/quizzes/results/learn-by-activities/${activity}/do-quizzes/lessons/${lesson.id}?activity_type=multiple_choice`}
