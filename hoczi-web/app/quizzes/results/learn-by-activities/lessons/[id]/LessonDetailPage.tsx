...
function mapActivityToQuestion(a: Activity, index: number): Question {
...
export function LessonDetailPage({ id }: { id: number }) {
    ...
    const [slots, setSlots] = useState<(Word | null)[]>([]);
    ...
    useEffect(() => {
        LessonService.getActivitiesByLessonId(id)
            .then((res) => {
                const list: Question[] = (res?.data ?? res ?? []).map(mapActivityToQuestion);
                setQuestions(list);
                if (list.length > 0) initWithQuestion(list[0]);
            })
    ...
    function initWithQuestion(q: Question) {
        ...
        setSpeaking(false);
        const wordTexts = q.words && q.words.length > 0 ? q.words : buildWords(q.content).map((w) => w.text);
        const bankWords: Word[] = shuffle(wordTexts.map((text, i) => ({ id: `w-${i}-${text}`, text })));
        const slotCount = q.correct_order && q.correct_order.length > 0 ? q.correct_order.length : wordTexts.length;
        setBank(bankWords);
    ...
    function handleReset() {
        initWithQuestion(questions[current]);
    }
