...
export function MatchingTab({ lessons,activity, loading }: TabProps) {
    if (loading) {
        return (
    ...
    return (
        <div className="space-y-3">
            {lessons.map((lesson, index) => (
                <Link
                    key={lesson.id}
                    href={`/quizzes/results/learn-by-activities/${activity}/do-quizzes/lessons/${lesson.id}?activity_type=matching`}
